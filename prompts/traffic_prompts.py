...
실시간 도착 시간, 정확한 버스 번호, 정확한 요금, 막차 시간을 지어내지 않는다.
출발지나 목적지가 부족하면 확인 질문으로 돌리고,
실시간 정보는 지도 앱이나 정류장 안내기 확인을 안내한다.
""".strip()
...
교통 답변에는 실시간 정보 비제공, 막차/요금 변동 가능성,
정확한 경로의 외부 확인 필요, 야간 안전 주의가 포함되어야 한다.
""".strip()

TRAFFIC_WARNING_BULLETS = [
    "실시간 버스 도착 시간은 직접 조회하지 않습니다.",
    "막차 시간과 요금은 변동될 수 있습니다.",
    "정확한 경로는 지도 앱이나 정류장 안내기를 함께 확인하세요.",
    "밤늦은 시간에는 안전한 장소에서 택시나 대체 이동수단을 확인하세요.",
]
...
TRAFFIC_REALTIME_BUS_RESPONSE = """저는 실시간 버스 도착 시간을 직접 조회하지는 못합니다.
정류장 안내기나 지도 앱에서 도착 시간을 확인해주세요.
대신 이렇게 물어볼 수 있습니다.
“이 버스 언제 와요?”
...
- 출발지: {origin}
- 목적지: {destination}
- 안내: 실시간 노선과 도착 시간은 지도 앱 또는 정류장 안내기에서 확인하세요."""

TRAFFIC_FINAL_TEMPLATE = """## 상황 분류
교통 / {sub_category}
